...
func ID003(strx string) []string {
	...
	switch {
	case value <= 1:
		return []string{"pass", "未觸發", "-1", "自動核銷"}
	case value > 71:
		return []string{"High", "ID003告警1", "1", "自動核銷"}
	case value > 41:
		return []string{"Medium", "ID003告警2", "2", "自動核銷"}
	case value >= 1:
		return []string{"Low", "ID003告警3", "3", "自動核銷"}
	}
	return []string{"undefined", "", "", ""}
}
